/* Leonid Lysenko st128618@student.spbu.ru
   Lab3
*/

#pragma once

#include <vector>
#include <random>
#include <iterator>
#include <stdexcept>
#include <algorithm>
#include <ctime>
#include <initializer_list>

/**
 * @brief A probabilistic, ordered key-value container based on the skip list data structure.
 * 
 * @tparam KeyType Type of keys.
 * @tparam ValueType Type of values.
 * @tparam Allocator Allocator type for node allocation.
 */
template <typename KeyType, typename ValueType, typename Allocator = std::allocator<std::pair<const KeyType, ValueType>>>
class SkipList {

private:
    /**
     * @brief Internal node structure for SkipList.
     */
    struct SkipNode {
        KeyType node_key;                          ///< Key stored in the node.
        ValueType node_value;                      ///< Value stored in the node.
        SkipNode* back_link;                       ///< Pointer to the previous node at level 0.
        std::vector<SkipNode*> forward_links;      ///< Array of forward pointers for each level.

        /**
         * @brief Construct a new SkipNode object.
         * 
         * @param k Key to store.
         * @param v Value to store.
         * @param prev Pointer to the previous node at level 0.
         * @param levels Number of levels for this node.
         */
        SkipNode(const KeyType& k, const ValueType& v, SkipNode* prev, size_t levels)
            : node_key(k), node_value(v), back_link(prev), forward_links(levels, nullptr) {}
    };

    size_t max_height;                             ///< Maximum height of the skip list.
    double promotion_chance;                       ///< Probability to promote a node to a higher level.
    size_t current_height;                         ///< Current maximum height of the skip list.
    size_t item_count;                             ///< Number of elements in the skip list.
    std::mt19937 rng_engine;                       ///< Random number generator engine.
    std::uniform_real_distribution<> probability_dist; ///< Distribution for promotion probability.
    Allocator data_allocator;                      ///< Allocator for node memory management.
    SkipNode* header;                              ///< Pointer to the header node.
    SkipNode* terminator;                          ///< Pointer to the terminator node.

    /**
     * @brief Generate a random level for a new node based on promotion probability.
     * 
     * @return size_t The generated level.
     */
    size_t generate_random_level() {
        size_t lvl = 1;
        while (probability_dist(rng_engine) < promotion_chance && lvl < max_height) {
            lvl++;
        }
        return lvl;
    }

    /**
     * @brief Locate a node by key.
     * 
     * @param search_key Key to search for.
     * @return SkipNode* Pointer to the node if found, nullptr otherwise.
     */
    SkipNode* locate_node(const KeyType& search_key) const {
        SkipNode* current = header;
        for (int lvl = current_height - 1; lvl >= 0; --lvl) {
            while (current->forward_links[lvl] != terminator && current->forward_links[lvl]->node_key < search_key) {
                current = current->forward_links[lvl];
            }
        }
        current = current->forward_links[0];
        return (current != terminator && current->node_key == search_key) ? current : nullptr;
    }

    /**
     * @brief Find predecessor nodes for a given key at each level.
     * 
     * @param search_key Key to search for.
     * @return std::vector<SkipNode*> Vector of predecessor nodes at each level.
     */
    std::vector<SkipNode*> find_predecessors(const KeyType& search_key) const {
        std::vector<SkipNode*> preds(current_height, nullptr);
        SkipNode* current = header;
        for (int lvl = current_height - 1; lvl >= 0; --lvl) {
            while (current->forward_links[lvl] != terminator &&
                   current->forward_links[lvl]->node_key < search_key) {
                current = current->forward_links[lvl];
            }
            preds[lvl] = current;
        }
        return preds;
    }

public:
    /**
     * @brief Bidirectional iterator for SkipList.
     */
    class iterator {
    public:
        using iterator_category = std::bidirectional_iterator_tag;
        using value_type = ValueType;
        using difference_type = std::ptrdiff_t;
        using pointer = ValueType*;
        using reference = ValueType&;

        /**
         * @brief Default constructor.
         */
        iterator() : container(nullptr), current_node(nullptr) {}

        /**
         * @brief Construct an iterator for a given node.
         * 
         * @param cont Pointer to the containing SkipList.
         * @param node Pointer to the node.
         */
        iterator(SkipList* cont, SkipNode* node) : container(cont), current_node(node) {}

        /**
         * @brief Dereference operator.
         * 
         * @return Reference to the value.
         * @throws std::out_of_range If the iterator is invalid.
         */
        reference operator*() const {
            if (!current_node || current_node == container->header)
                throw std::out_of_range("Invalid iterator dereference");
            return current_node->node_value;
        }

        /**
         * @brief Arrow operator.
         * 
         * @return Pointer to the value.
         */
        pointer operator->() const {
            return &(operator*());
        }

        /**
         * @brief Pre-increment operator.
         * 
         * @return Reference to this iterator.
         */
        iterator& operator++() {
            if (current_node) {
                current_node = current_node->forward_links[0];
                if (current_node == container->terminator) {
                    current_node = nullptr;
                }
            }
            return *this;
        }

        /**
         * @brief Post-increment operator.
         * 
         * @return Iterator before increment.
         */
        iterator operator++(int) {
            iterator temp = *this;
            ++(*this);
            return temp;
        }

        /**
         * @brief Pre-decrement operator.
         * 
         * @return Reference to this iterator.
         */
        iterator& operator--() {
            if (current_node) {
                current_node = current_node->back_link;
                if (current_node == container->header) {
                    current_node = nullptr;
                }
            } else {
                current_node = container->terminator->back_link;
                if (current_node == container->header) {
                    current_node = nullptr;
                }
            }
            return *this;
        }

        /**
         * @brief Post-decrement operator.
         * 
         * @return Iterator before decrement.
         */
        iterator operator--(int) {
            iterator temp = *this;
            --(*this);
            return temp;
        }

        /**
         * @brief Equality comparison.
         * 
         * @param other Iterator to compare with.
         * @return true If equal.
         * @return false Otherwise.
         */
        bool operator==(const iterator& other) const {
            return current_node == other.current_node;
        }

        /**
         * @brief Inequality comparison.
         * 
         * @param other Iterator to compare with.
         * @return true If not equal.
         * @return false Otherwise.
         */
        bool operator!=(const iterator& other) const {
            return !(*this == other);
        }

        /**
         * @brief Get the key of the current element.
         * 
         * @return KeyType Key of the element.
         * @throws std::out_of_range If the iterator is invalid.
         */
        KeyType key() const {
            if (!current_node) throw std::out_of_range("Invalid key access");
            return current_node->node_key;
        }

    private:
        SkipList* container;        ///< Pointer to the container.
        SkipNode* current_node;     ///< Pointer to the current node.
        friend class SkipList;
    };

    /**
     * @brief Bidirectional const iterator for SkipList.
     */
    class const_iterator {
    public:
        using iterator_category = std::bidirectional_iterator_tag;
        using value_type = const ValueType;
        using difference_type = std::ptrdiff_t;
        using pointer = const ValueType*;
        using reference = const ValueType&;

        /**
         * @brief Default constructor.
         */
        const_iterator() : container(nullptr), current_node(nullptr) {}

        /**
         * @brief Construct a const_iterator for a given node.
         * 
         * @param cont Pointer to the containing SkipList.
         * @param node Pointer to the node.
         */
        const_iterator(const SkipList* cont, const SkipNode* node)
            : container(cont), current_node(node) {}

        /**
         * @brief Construct a const_iterator from a non-const iterator.
         * 
         * @param other Non-const iterator.
         */
        const_iterator(const iterator& other)
            : container(other.container), current_node(other.current_node) {}

        /**
         * @brief Dereference operator.
         * 
         * @return Reference to the value.
         * @throws std::out_of_range If the iterator is invalid.
         */
        reference operator*() const {
            if (!current_node || current_node == container->header)
                throw std::out_of_range("Invalid const_iterator dereference");
            return current_node->node_value;
        }

        /**
         * @brief Arrow operator.
         * 
         * @return Pointer to the value.
         */
        pointer operator->() const {
            return &(operator*());
        }

        /**
         * @brief Pre-increment operator.
         * 
         * @return Reference to this iterator.
         */
        const_iterator& operator++() {
            if (current_node) {
                current_node = current_node->forward_links[0];
                if (current_node == container->terminator) {
                    current_node = nullptr;
                }
            }
            return *this;
        }

        /**
         * @brief Post-increment operator.
         * 
         * @return Iterator before increment.
         */
        const_iterator operator++(int) {
            const_iterator temp = *this;
            ++(*this);
            return temp;
        }

        /**
         * @brief Pre-decrement operator.
         * 
         * @return Reference to this iterator.
         */
        const_iterator& operator--() {
            if (current_node) {
                current_node = current_node->back_link;
                if (current_node == container->header) {
                    current_node = nullptr;
                }
            } else {
                current_node = container->terminator->back_link;
                if (current_node == container->header) {
                    current_node = nullptr;
                }
            }
            return *this;
        }

        /**
         * @brief Post-decrement operator.
         * 
         * @return Iterator before decrement.
         */
        const_iterator operator--(int) {
            const_iterator temp = *this;
            --(*this);
            return temp;
        }

        /**
         * @brief Equality comparison.
         * 
         * @param other Iterator to compare with.
         * @return true If equal.
         * @return false Otherwise.
         */
        bool operator==(const const_iterator& other) const {
            return current_node == other.current_node;
        }

        /**
         * @brief Inequality comparison.
         * 
         * @param other Iterator to compare with.
         * @return true If not equal.
         * @return false Otherwise.
         */
        bool operator!=(const const_iterator& other) const {
            return !(*this == other);
        }

        /**
         * @brief Get the key of the current element.
         * 
         * @return const KeyType& Key of the element.
         * @throws std::out_of_range If the iterator is invalid.
         */
        const KeyType& key() const {
            if (!current_node) throw std::out_of_range("Invalid key access");
            return current_node->node_key;
        }

    private:
        const SkipList* container;      ///< Pointer to the container.
        const SkipNode* current_node;   ///< Pointer to the current node.
        friend class SkipList;
    };

    using reverse_iterator = std::reverse_iterator<iterator>;                  ///< Reverse iterator type.
    using const_reverse_iterator = std::reverse_iterator<const_iterator>;      ///< Const reverse iterator type.

    /**
     * @brief Construct a new SkipList object.
     * 
     * @param height_limit Maximum height of the skip list.
     * @param promotion_prob Probability to promote a node to a higher level.
     * @param alloc Allocator for node memory.
     */
    SkipList(size_t height_limit = 16, double promotion_prob = 0.5,
             const Allocator& alloc = Allocator())
        : max_height(height_limit),
          promotion_chance(promotion_prob),
          current_height(1),
          item_count(0),
          rng_engine(std::time(nullptr)),
          probability_dist(0.0, 1.0),
          data_allocator(alloc) {
        header = new SkipNode(KeyType(), ValueType(), nullptr, max_height);
        terminator = new SkipNode(KeyType(), ValueType(), header, max_height);
        for (size_t lvl = 0; lvl < max_height; ++lvl) {
            header->forward_links[lvl] = terminator;
        }
        terminator->back_link = header;
    }

    /**
     * @brief Construct a new SkipList object from an initializer list.
     * 
     * @param init_list Initializer list of key-value pairs.
     * @param height_limit Maximum height of the skip list.
     * @param promotion_prob Probability to promote a node to a higher level.
     */
    SkipList(std::initializer_list<std::pair<KeyType, ValueType>> init_list,
             size_t height_limit = 16, double promotion_prob = 0.5)
        : SkipList(height_limit, promotion_prob) {
        for (const auto& pair : init_list) {
            insert(pair.first, pair.second);
        }
    }

    /**
     * @brief Copy constructor.
     * 
     * @param other SkipList to copy from.
     */
    SkipList(const SkipList& other)
        : max_height(other.max_height),
          promotion_chance(other.promotion_chance),
          current_height(1),
          item_count(0),
          rng_engine(std::time(nullptr)),
          probability_dist(0.0, 1.0),
          data_allocator(other.data_allocator) {
        header = new SkipNode(KeyType(), ValueType(), nullptr, max_height);
        terminator = new SkipNode(KeyType(), ValueType(), header, max_height);
        for (size_t lvl = 0; lvl < max_height; ++lvl) {
            header->forward_links[lvl] = terminator;
        }
        for (auto it = other.begin(); it != other.end(); ++it) {
            insert(it.key(), *it);
        }
    }

    /**
     * @brief Copy assignment operator.
     * 
     * @param other SkipList to assign from.
     * @return Reference to this SkipList.
     */
    SkipList& operator=(const SkipList& other) {
        if (this != &other) {
            SkipList temp(other);
            swap(temp);
        }
        return *this;
    }

    /**
     * @brief Destructor. Destroys all nodes and releases memory.
     */
    ~SkipList() {
        purge();
        delete header;
        delete terminator;
    }

    /**
     * @brief Get iterator to the first element.
     * 
     * @return iterator Iterator to the first element.
     */
    iterator begin() noexcept {
        return iterator(this, header->forward_links[0] != terminator ? header->forward_links[0] : nullptr);
    }

    /**
     * @brief Get iterator to the end (past-the-last) element.
     * 
     * @return iterator Iterator to the end.
     */
    iterator end() noexcept {
        return iterator(this, nullptr);
    }

    /**
     * @brief Get const_iterator to the first element.
     * 
     * @return const_iterator Const iterator to the first element.
     */
    const_iterator begin() const noexcept {
        return const_iterator(this, header->forward_links[0] != terminator ? header->forward_links[0] : nullptr);
    }

    /**
     * @brief Get const_iterator to the end (past-the-last) element.
     * 
     * @return const_iterator Const iterator to the end.
     */
    const_iterator end() const noexcept {
        return const_iterator(this, nullptr);
    }

    /**
     * @brief Get const_iterator to the first element.
     * 
     * @return const_iterator Const iterator to the first element.
     */
    const_iterator cbegin() const noexcept {
        return begin();
    }

    /**
     * @brief Get const_iterator to the end (past-the-last) element.
     * 
     * @return const_iterator Const iterator to the end.
     */
    const_iterator cend() const noexcept {
        return end();
    }

    /**
     * @brief Get reverse_iterator to the last element.
     * 
     * @return reverse_iterator Reverse iterator to the last element.
     */
    reverse_iterator rbegin() noexcept {
        return reverse_iterator(end());
    }

    /**
     * @brief Get reverse_iterator to the element before the first.
     * 
     * @return reverse_iterator Reverse iterator to before the first element.
     */
    reverse_iterator rend() noexcept {
        return reverse_iterator(begin());
    }

    /**
     * @brief Get const_reverse_iterator to the last element.
     * 
     * @return const_reverse_iterator Const reverse iterator to the last element.
     */
    const_reverse_iterator rbegin() const noexcept {
        return const_reverse_iterator(end());
    }

    /**
     * @brief Get const_reverse_iterator to the element before the first.
     * 
     * @return const_reverse_iterator Const reverse iterator to before the first element.
     */
    const_reverse_iterator rend() const noexcept {
        return const_reverse_iterator(begin());
    }

    /**
     * @brief Get const_reverse_iterator to the last element.
     * 
     * @return const_reverse_iterator Const reverse iterator to the last element.
     */
    const_reverse_iterator crbegin() const noexcept {
        return const_reverse_iterator(end());
    }

    /**
     * @brief Get const_reverse_iterator to the element before the first.
     * 
     * @return const_reverse_iterator Const reverse iterator to before the first element.
     */
    const_reverse_iterator crend() const noexcept {
        return const_reverse_iterator(begin());
    }

    /**
     * @brief Insert a key-value pair into the skip list.
     * 
     * @param new_key Key to insert.
     * @param new_value Value to insert.
     */
    void insert(const KeyType& new_key, const ValueType& new_value) {
        auto predecessors = find_predecessors(new_key);
        if (predecessors[0]->forward_links[0] != terminator &&
            predecessors[0]->forward_links[0]->node_key == new_key) {
            predecessors[0]->forward_links[0]->node_value = new_value;
            return;
        }
        size_t new_level = generate_random_level();
        if (new_level > current_height) {
            predecessors.resize(new_level, header);
            current_height = new_level;
        }
        SkipNode* new_node = new SkipNode(new_key, new_value, predecessors[0], new_level);
        for (size_t lvl = 0; lvl < new_level; ++lvl) {
            new_node->forward_links[lvl] = predecessors[lvl]->forward_links[lvl];
            predecessors[lvl]->forward_links[lvl] = new_node;
        }
        new_node->back_link = predecessors[0];
        if (new_node->forward_links[0] != terminator) {
            new_node->forward_links[0]->back_link = new_node;
        } else {
            terminator->back_link = new_node;
        }
        item_count++;
    }

    /**
     * @brief Erase a key-value pair by key.
     * 
     * @param key Key to erase.
     */
    void erase(const KeyType& key) {
        auto predecessors = find_predecessors(key);
        SkipNode* target = predecessors[0]->forward_links[0];
        if (target == terminator || target->node_key != key) return;
        for (size_t lvl = 0; lvl < target->forward_links.size(); ++lvl) {
            if (predecessors[lvl]->forward_links[lvl] == target) {
                predecessors[lvl]->forward_links[lvl] = target->forward_links[lvl];
            }
        }
        if (target->forward_links[0] != terminator) {
            target->forward_links[0]->back_link = target->back_link;
        } else {
            terminator->back_link = target->back_link;
        }
        delete target;
        item_count--;
        while (current_height > 1 && header->forward_links[current_height-1] == terminator) {
            current_height--;
        }
    }

    /**
     * @brief Find an iterator to the element with the given key.
     * 
     * @param key Key to find.
     * @return iterator Iterator to the element, or end() if not found.
     */
    iterator find(const KeyType& key) {
        SkipNode* node = locate_node(key);
        return iterator(this, node);
    }

    /**
     * @brief Find a const_iterator to the element with the given key.
     * 
     * @param key Key to find.
     * @return const_iterator Const iterator to the element, or end() if not found.
     */
    const_iterator find(const KeyType& key) const {
        SkipNode* node = locate_node(key);
        return const_iterator(this, node);
    }

    /**
     * @brief Access or insert element by key.
     * 
     * @param key Key to access or insert.
     * @return ValueType& Reference to the value.
     */
    ValueType& operator[](const KeyType& key) {
        SkipNode* node = locate_node(key);
        if (!node) {
            insert(key, ValueType());
            node = locate_node(key);
        }
        return node->node_value;
    }

    /**
     * @brief Access element by key (const).
     * 
     * @param key Key to access.
     * @return const ValueType& Reference to the value.
     */
    const ValueType& operator[](const KeyType& key) const {
        return at(key);
    }

    /**
     * @brief Access element by key with bounds checking.
     * 
     * @param key Key to access.
     * @return const ValueType& Reference to the value.
     * @throws std::out_of_range If key is not found.
     */
    const ValueType& at(const KeyType& key) const {
        const SkipNode* node = locate_node(key);
        if (!node) throw std::out_of_range("Key not found");
        return node->node_value;
    }

    /**
     * @brief Get the number of elements in the skip list.
     * 
     * @return size_t Number of elements.
     */
    size_t size() const noexcept {
        return item_count;
    }

    /**
     * @brief Check if the skip list is empty.
     * 
     * @return true If empty.
     * @return false Otherwise.
     */
    bool empty() const noexcept {
        return item_count == 0;
    }

    /**
     * @brief Remove all elements from the skip list.
     */
    void purge() noexcept {
        SkipNode* current = header->forward_links[0];
        while (current != terminator) {
            SkipNode* next = current->forward_links[0];
            delete current;
            current = next;
        }
        for (size_t lvl = 0; lvl < max_height; ++lvl) {
            header->forward_links[lvl] = terminator;
        }
        terminator->back_link = header;
        current_height = 1;
        item_count = 0;
    }

    /**
     * @brief Swap contents with another SkipList.
     * 
     * @param other SkipList to swap with.
     */
    void swap(SkipList& other) noexcept {
        std::swap(max_height, other.max_height);
        std::swap(promotion_chance, other.promotion_chance);
        std::swap(current_height, other.current_height);
        std::swap(item_count, other.item_count);
        std::swap(header, other.header);
        std::swap(terminator, other.terminator);
        std::swap(data_allocator, other.data_allocator);
    }

    /**
     * @brief Less-than comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If lhs is less than rhs.
     * @return false Otherwise.
     */
    friend bool operator<(const SkipList& lhs, const SkipList& rhs) {
        auto lhs_it = lhs.begin();
        auto rhs_it = rhs.begin();
        while (lhs_it != lhs.end() && rhs_it != rhs.end()) {
            if (lhs_it.key() < rhs_it.key()) return true;
            if (rhs_it.key() < lhs_it.key()) return false;
            if (*lhs_it < *rhs_it) return true;
            if (*rhs_it < *lhs_it) return false;
            ++lhs_it;
            ++rhs_it;
        }
        return lhs.size() < rhs.size();
    }

    /**
     * @brief Greater-than comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If lhs is greater than rhs.
     * @return false Otherwise.
     */
    friend bool operator>(const SkipList& lhs, const SkipList& rhs) {
        return rhs < lhs;
    }

    /**
     * @brief Less-than-or-equal comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If lhs is less than or equal to rhs.
     * @return false Otherwise.
     */
    friend bool operator<=(const SkipList& lhs, const SkipList& rhs) {
        return !(rhs < lhs);
    }

    /**
     * @brief Greater-than-or-equal comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If lhs is greater than or equal to rhs.
     * @return false Otherwise.
     */
    friend bool operator>=(const SkipList& lhs, const SkipList& rhs) {
        return !(lhs < rhs);
    }

    /**
     * @brief Equality comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If both SkipLists are equal.
     * @return false Otherwise.
     */
    friend bool operator==(const SkipList& lhs, const SkipList& rhs) {
        if (lhs.size() != rhs.size()) return false;
        auto lit = lhs.begin();
        auto rit = rhs.begin();
        while (lit != lhs.end() && rit != rhs.end()) {
            if (*lit != *rit)
                return false;
            ++lit;
            ++rit;
        }
        return true;
    }

    /**
     * @brief Inequality comparison operator.
     * 
     * @param lhs Left-hand side SkipList.
     * @param rhs Right-hand side SkipList.
     * @return true If SkipLists are not equal.
     * @return false Otherwise.
     */
    friend bool operator!=(const SkipList& lhs, const SkipList& rhs) {
        return !(lhs == rhs);
    }
};

